...
class Baseline(nn.Module):
    def __init__(self, num_classes=None, drop_last_stride=False, decompose=False, **kwargs):
        ...
        self.use_rsca = kwargs.get('use_rsca', True)
        ...
        if self.use_lab:
            self.lab_module = LabAlignmentModule(
                use_channel_weight=kwargs.get('lab_channel_weight', True)
            )
    ...
    def forward(self, inputs, labels=None, **kwargs):
        ...
        sh_feat, sh_pl, sp_pl, sp_IN,sp_IN_p,x_sp_f,x_sp_f_p = self.backbone(inputs,sub=sub)
        #提取特征
        ...
        if not self.training:
            ...
            return self.train_forward(feats, sp_pl, labels,
                                       sub, sp_IN,sp_IN_p,x_sp_f,x_sp_f_p,sh_feat_map=sh_feat, **kwargs)



    def train_forward(self, feat, sp_pl, labels,
                       sub, sp_IN,sp_IN_p,x_sp_f,x_sp_f_p,sh_feat_map=None, **kwargs):
        epoch = kwargs.get('epoch')
        metric = {}
        loss = 0
        ...
        # ===== 新增：rSCA损失 =====
        if self.use_rsca :
            rsca_loss = self.backbone.rsca_loss
            # 加权系数（推荐0.1-0.5）
            rsca_weight = kwargs.get('rsca_weight', 0.2)
            loss += rsca_weight * rsca_loss
            metric.update({'rsca': rsca_loss.data})

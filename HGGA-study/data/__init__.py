...
# 第7行左右，添加：
from data.transforms_lab import RandomColoring_Lab
...
def get_train_loader(dataset, root, sample_method, batch_size, p_size, k_size, image_size, random_flip=False, random_crop=False,
    ...
    if True==False: #tsne 注释
        ...
        # ===== 1. 转 Tensor =====
        t.append(T.ToTensor())  # [0, 1]

        # ===== 2. Lab-DMT (颜色/亮度增强) =====
        use_lab_dmt = True  # 你的开关
        if use_lab_dmt:
            t.append(RandomColoring_Lab(p=0.5))
            print("[INFO] Lab-DMT enabled")

        # ===== 3. RandomErasing (局部擦除) =====
        if random_erase:
            t.append(T.RandomErasing())  # ✅ 在归一化之前

        # ===== 4. Normalize (最后标准化) =====
        t.append(T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]))
